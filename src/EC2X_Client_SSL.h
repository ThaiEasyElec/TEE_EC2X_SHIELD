#ifndef EC2XClientSSL_h
#define EC2XClientSSL_h
...
class EC2XClientSSL : public Stream
{
	public:
		uint8_t buffer[Client_SSL_SIZE];
		EC2XClientSSL(uint8_t connectID,uint8_t sslID){
			
			sslID_ = sslID;
			tcp = new EC2X_TCP_SSL(connectID);
		};
		
		bool connect() 
		{
			memset(buffer,0,Client_SSL_SIZE);
			return  tcp->open(host,port,0,sslID_);
		}
